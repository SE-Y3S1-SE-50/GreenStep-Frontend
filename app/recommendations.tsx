...
export default function RecommendationsSheet() {
  const { data } = useQuery({ queryKey: ['recommendations'], queryFn: getRecommendations });
  const qc = useQueryClient();
  const onJoin = async (id: string) => {
    const prev = qc.getQueryData<Challenge[]>(['challenges']);
    try {
      const list = Array.isArray(prev) ? prev : [];
      qc.setQueryData(['challenges'], list.map((c) => (c.id === id ? { ...c, joined: true } : c)));
      await joinChallenge(id);
      qc.invalidateQueries({ queryKey: ['challenges'] });
      qc.invalidateQueries({ queryKey: ['recommendations'] });
    } catch (e) {
      if (prev) qc.setQueryData(['challenges'], prev as any);
    }
  };
  return (
    <View style={{ padding: 16, flex: 1 }}>
      <Text style={{ fontWeight: '800', fontSize: 18, marginBottom: 12 }}>Recommended challenges</Text>
      <FlatList
        data={data || []}
        keyExtractor={(i) => i.id}
        renderItem={({ item }) => (
